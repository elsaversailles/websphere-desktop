...
export function TrackerPage({ notify }: { notify: Notice }) {
  ...
  const tasks = projects?.flatMap((project) => project.tasks.map((task, index) => ({ ...task, key: `${project.id}-${index}`, project: project.title }))) ?? [];
  const complete = tasks.filter((task) => task.status === 'completed').length;
  return <section className="sp on"><Head title="Task Progress Tracker" action={<div className="ph-acts"><button className="btn-o btn-sm" onClick={() => notify('Choose a project task before attaching a file link.')}>Attach File Link</button><button className="btn btn-sm" onClick={() => notify('Connected files synced.')}>Sync Files</button></div>} /><div className="krow"><Kpi value={tasks.length || '—'} label="Total Files" /><Kpi value={tasks.filter((task) => task.status === 'ongoing').length || '—'} label="In Progress" tone="yel" /><Kpi value={complete || '—'} label="Completed" tone="grn" /><Kpi value={tasks.length ? `${Math.round(complete / tasks.length * 100)}%` : '—'} label="Avg. Progress" /></div><div className="tracker-filter"><div>{['All Files','Not Started','In Progress','For Review','Completed'].map((label, index) => <button className={index === 0 ? 'on' : ''} key={label}>{label}</button>)}</div><select className="legacy-select"><option>All Platforms</option></select></div>{tasks.length ? <div className="file-card-grid">{tasks.map((task) => <article className="file-card" key={task.key}><div className="file-icon">▤</div><div><strong>{task.project}</strong><span>Your linked task file appears here.</span></div><Badge value={task.status} /></article>)}</div> : <div className="cc"><Empty>Files connected to your projects will appear here.</Empty></div>}<div className="g2"><InfoCard title="Recent File Activity" text="Your recent file activity will appear here." /><InfoCard title="Active Collaborators" text="Group collaborators will appear here." /></div></section>;
}
