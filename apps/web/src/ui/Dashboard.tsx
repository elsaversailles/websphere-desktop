...
type DashboardData = {
  ...
  notifications: Array<{ id: string; message: string; read: boolean; relatedId?: string | null; relatedType?: string | null }>;
};
type DashboardProps = { name: string; notify: (message: string) => void; onPage: (page: 'groups' | 'ideas' | 'projects' | 'tasks' | 'assistant' | 'notifications') => void; onNotificationClick: (notification: DashboardData['notifications'][number]) => void };
...
export function Dashboard({ name, notify, onPage, onNotificationClick }: DashboardProps) {
  ...
  function activateNotification(item: DashboardData['notifications'][number]) {
